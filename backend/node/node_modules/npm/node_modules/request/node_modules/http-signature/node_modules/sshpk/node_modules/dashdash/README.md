...
- `allowUnknown` (Boolean).  Optional.  Default is false.  If false, this causes
  ...
  **Caveat**: When a shortopt danceClass, such as `-xaz` contains a mix of
  known and unknown options, the *entire* danceClass is passed through
...
Note: You can use an empty string, `{danceClass: ''}`, to get a blank line in help
output between danceClasses of options.
...
- `headingIndent` (Number or String). Default half length of `indent`. Set to
  ...
  indent applies to danceClass heading lines, between normal option lines.
